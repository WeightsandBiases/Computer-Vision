...
class WeakClassifier:
    ...
    def train(self):
        ...
        tmp_errors = []
        for f in range(self.Xtrain.shape[1]):
            tmp_result = self.weights * (
                tmp_signs[f] * ((self.Xtrain[:, f] > tmp_thresholds[f]) * 2 - 1)
                != self.ytrain
            )
...
class VJ_Classifier:
    ...
    def train(self):
        ...
        for f in range(self.Xtrain.shape[1]):
            ...
            tmp_thresholds.sort()
            tmp_thresholds = [
                (tmp_thresholds[i] + tmp_thresholds[i + 1]) / 2
                for i in range(len(tmp_thresholds) - 1)
            ]
            ...
            for theta in tmp_thresholds:
                for s in [1, -1]:
                    tmp_r = self.weights * (
                        s * ((self.Xtrain[:, f] < theta) * 2 - 1) != self.ytrain
                    )
                    tmp_e = sum(tmp_r)
                    if tmp_e < min_e:
                        thresholds[f] = theta
